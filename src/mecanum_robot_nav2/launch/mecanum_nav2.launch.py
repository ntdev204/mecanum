...
def generate_launch_description():
    ...
    mecanum_robot_dir = get_package_share_directory('turn_on_mecanum_robot')
    mecanum_launch_dir = os.path.join(mecanum_robot_dir, 'launch')
        
    mecanum_nav_dir = get_package_share_directory('mecanum_nav2')
    mecanum_nav_launchr = os.path.join(mecanum_nav_dir, 'launch')


    map_dir = os.path.join(mecanum_nav_dir, 'map')
    map_file = LaunchConfiguration('map', default=os.path.join(
        map_dir, 'mecanum.yaml'))
    ...
    param_dir = os.path.join(mecanum_nav_dir, 'param','mecanum_params')
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [mecanum_launch_dir, '/turn_on_mecanum_robot.launch.py']),
        ),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [mecanum_launch_dir, '/mecanum_lidar.launch.py']),
        ),        
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [mecanum_nav_launchr, '/bringup_launch.py']),
            launch_arguments={
                'map': map_file,
